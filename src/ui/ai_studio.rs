...
impl StudioApp {
    pub(crate) fn view_ai_studio(&mut self, ui: &mut egui::Ui) {
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                ...
                ui.group(|ui| {
                    ...
                    ui.label(
                        egui::RichText::new(
                         ...
                         before the playlist is created. (Spotify's own recommendation \
                         endpoints were retired for new apps in Nov 2024 — the AI is the \
                ...
                // ---------------- Refine ----------------
                ui.group(|ui| {
                    ui.strong("Refine a playlist (\"more like this, less like that\")");
                    let selected = self.selected.clone();
                    ui.horizontal(|ui| {
                        ui.label("Target:");
                        self.source_picker(ui, "refine-source");
                    });
                    ...
                    match &selected {
                        ...
                        Some(_) => {
                            ...
                            ui.label(
                                egui::RichText::new(
                                    "Protected source — the refinement will be written to a NEW \
                                 playlist; the original is never touched.",
                                )
                                .weak()
                                .small(),
                            );
                    ...
                    let can =
                        ready && selected.is_some() && !self.refine_instruction.trim().is_empty();
                    if ui
                ...
                ui.group(|ui| {
                    ...
                    ui.label(
                        egui::RichText::new(
                         ...
                         untouched — everything lands in NEW playlists.",
                        )
                        .weak()
                        .small(),
                    );
                    ui.add(
                        egui::TextEdit::multiline(&mut self.organize_goal)
                            .hint_text(
                             ...
                             \"build me better workout / focus / dinner playlists from what I \
                             already like\"",
                            )
                            .desired_rows(2)
                            .desired_width(f32::INFINITY),
    ...
    pub(crate) fn source_picker(&mut self, ui: &mut egui::Ui, salt: &str) {
        let current = self
            .selected
            .as_ref()
            .map(|s| s.label())
